class MonacoEditorApp {
    ...
    renderDashboard() {
        ...
        this.projects.forEach(project => {
            ...
            card.innerHTML = `
                ...
                <div class="project-name" title="${project.name}">${project.name.length > 11 ? project.name.slice(0, 11) + '...' : project.name}</div>
            `;
    ...
    createNewProject() {
        if (this.projects.length >= 43) {
            show('Max projects reached, delete a project to create more', 6000);
            return;
        }
...
`,
    ...
    console.log(
        "%cDebug",
        "color:dodgerblue; font-size:28px; font-weight:bold;"
    );

    console.log(
        "%cDeveloper Console • Diagnostics & Logging",
        "color:dodgerblue; font-size:14px; opacity:0.8;"
    );
    if (app && app.editor) {
...
function checkSize(e) {
    if (e.matches) {
        alert("Site is not yet availiable on mobile, try Tablet or PC");
    }
}
...
downloadbutton.addEventListener("click", async function () {
    ...
    try {
        const backendUrl = "https://persistent-hosting--nipanzuzuzu9.replit.app/generate-ipa";
        ...
        // Attach project icon if one has been set
        if (project.icon && project.icon.startsWith('data:')) {
            const iconRes = await fetch(project.icon);
            const iconBlob = await iconRes.blob();
            formData.append('projectImage', iconBlob, 'icon.png');
        }
    ...
    mutations.forEach((mutation) => {
        mutation.addedNodes.forEach((node) => {
            if (node.nodeType === 1) {
                ...
                node.querySelectorAll && node.querySelectorAll('.project-card').forEach(el => {
...
async function aiGenerate() {
    ...
    // Need an active file + editor
    if (!app || !app.editor || !app.currentFile) {
        show('Open a file first, then use AI.');
        return;
    }

    // Detect language from current file key (e.g. "html/index.html")
    const fileKey = app.currentFile;
    const folder = fileKey.split('/')[0]; // html | css | js
    const langMap = { html: 'HTML', css: 'CSS', js: 'JavaScript' };
    const langName = langMap[folder] || 'code';

    btn.disabled = true;
    btn.classList.add('loading');
    show('AI thinking…', 60000);
    ...
    try {
        ...
        show('AI error: ' + err.message, 3000);
    } finally {
        btn.disabled = false;
...
function toggleAiBar() {
    ...
    btn.style.borderColor = hidden ? '#30363d' : '#1f6feb';
    btn.style.color = hidden ? '#7d8590' : '#e6edf3';
}